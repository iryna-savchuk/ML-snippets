...
dataset.head()
dataset.info() #as can be seen, all attributes are numerical, except the "Date" field
dataset["Date"].value_counts() #finding out how many instances relate to the same date

#Looking for standard correlations within Dataset
corr_matrix = dataset.corr()
corr_matrix["TargetPrice"].sort_values(ascending = False)

#Visualising scatter matrix (Focusing only on the most promising attributes to predict TargetPrice)
...
def validate_regressor(regressor): 
    ...
    gss = GroupShuffleSplit(n_splits = number_of_splits, test_size = 0.2, random_state = 0) #splitting the data
    ...
    # average RMSE thoughout all data splits and Standard Deviation of RMSE thoughout all data splits
...
# Evaluating performance of Linear Reression Model built on BidPrice and FeaturePrice4 features only
X = dataset.iloc[:, [1,8]].values
regressor_2 = LinearRegression()
validate_regressor(regressor_2)

"""
Random Forest Model with 10 estimators demonstrated the best performance among the considered models, because: 
1) it produced the least average Test RMSE
2) it has rather close average Training RMSE and average Test RMSE
3) the deviations of Test RMSE values is relatively low
"""

#Final training the chosen model (RandomForestRegressor) on the whole dataset
